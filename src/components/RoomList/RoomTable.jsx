...
const RoomTable = ({hotelId}) => 
{
  ...
  useEffect(()=>{
    if (!loading && !error && data) {
      setAvailableRooms(data.map((room) => 
  ...
  const handleSelect = (roomId, e) => {
    ...
    const roomNumbersToAdd = availableRooms[roomId]?.slice(0, numberSelected).map(room => room.id) || [];
    ...
    if (numberSelected > 0) {
      ...
      const currentRoomIds = new Set(availableRooms[roomId]?.map(room => room.id));
            ...
            <tr key={room.id}>
              ...
              <td>
                <select onChange={(e) => handleSelect(index, e)}>
                        ...
                        {[...Array(room.roomNumbers.length + 1).keys()].map((number) => (
                          <option 
                          key={number} 
                          value={number}
                          >
